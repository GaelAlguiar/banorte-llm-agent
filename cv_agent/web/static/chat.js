(() => {
  ...
  function makeMessage(message, loading = false) { const row = document.createElement("article"); row.className = `message ${message.role}`; if (message.role === "assistant") { const avatar = document.createElement("span"); avatar.className = "message-avatar"; avatar.textContent = "A"; row.append(avatar); } const wrap = document.createElement("div"); const bubble = document.createElement("div"); bubble.className = "bubble"; if (loading) { bubble.innerHTML = '<span class="typing" aria-label="AIguiar AI está escribiendo"><i></i><i></i><i></i></span>'; } else { bubble.textContent = message.content; } wrap.append(bubble); if (message.role === "assistant" && !loading) { const details = evidenceDetails(message.evidence); if (details) wrap.append(details); const copy = document.createElement("button"); copy.type = "button"; copy.className = "copy-button"; copy.textContent = "Copiar respuesta"; copy.addEventListener("click", async () => { await navigator.clipboard.writeText(message.content); copy.textContent = "Copiada"; setTimeout(() => copy.textContent = "Copiar respuesta", 1500); }); wrap.append(copy); } row.append(wrap); return row; }
  ...
  function add(role, content, evidence = []) { const chat = current(); chat.messages.push({ role, content, evidence }); if (chat.messages.length === 1) chat.title = content.slice(0, 46) + (content.length > 46 ? "…" : ""); chat.updatedAt = Date.now(); conversations.sort((a, b) => b.updatedAt - a.updatedAt); save(); render(); }
  function setBusy(value) { busy = value; elements.send.disabled = value; elements.input.disabled = value; }
  async function send(message) { const clean = message.trim(); if (!clean || busy) return; add("user", clean); elements.input.value = ""; resize(); setBusy(true); elements.messages.append(makeMessage({ role: "assistant", content: "" }, true)); try { const response = await fetch("/chat/api/messages", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: clean }) }); const data = await response.json(); if (!response.ok) throw new Error(data?.error?.message || "No fue posible procesar el mensaje."); add("assistant", data.response, data.evidence); } catch (error) { add("assistant", `No pude responder en este momento. ${error.message} Intenta nuevamente.`); } finally { setBusy(false); elements.input.focus(); } }
